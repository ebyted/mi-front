def extract_unique_categories_from_csv(csv_path):
    """Extrae todas las categorías únicas del archivo cat_art.csv"""
    categories = set()
    ...
    with open(csv_path, encoding='utf-8') as f:
        ...
        for row in reader:
            # Buscar columna de categoría (puede ser 'Categoría' o 'Categoria')
            cat_name = row.get('Categoría') or row.get('Categoria')
            if cat_name:
                cat_name = cat_name.strip()
                if cat_name and cat_name not in ['', 'N/A', 'NULL', 'null']:
...
def migrate_cat_art_csv():
    # Borra productos, categorías y marcas antes de migrar
    Product.objects.all().delete()
    Category.objects.all().delete()
    Brand.objects.all().delete()
    print("Productos, categorías y marcas han sido eliminados antes de la migración.")
...
if __name__ == "__main__":
    # Extrae e imprime categorías únicas del CSV
    csv_path = get_csv_path()
    extract_unique_categories_from_csv(csv_path)
