...
# ViewSet simplificado para productos
class SimpleProductViewSet(viewsets.ModelViewSet):
    """ViewSet simplificado para depurar problemas con productos"""
    permission_classes = [IsAuthenticated]
    ...
    def list(self, request, *args, **kwargs):
        try:
            # Obtener datos básicos sin serialización compleja
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            # Crear el producto directamente sin serializer complejo
            product = Product.objects.create(
                name=data.get('name'),
                sku=data.get('sku'),
                barcode=data.get('barcode', ''),
                minimum_stock=data.get('minimum_stock') if data.get('minimum_stock') else None,
                maximum_stock=data.get('maximum_stock') if data.get('maximum_stock') else None,
                is_active=data.get('is_active', True),
                group=data.get('group') if data.get('group') else None,
                brand_id=data.get('brand') if data.get('brand') else None,
                category_id=data.get('category') if data.get('category') else None,
                business_id=data.get('business') if data.get('business') else 1
            )
            
            # Devolver los datos básicos del producto creado
            response_data = {
                'id': product.id,
                'name': product.name,
                'sku': product.sku,
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            product.minimum_stock = data.get('minimum_stock') if data.get('minimum_stock') else None
            product.maximum_stock = data.get('maximum_stock') if data.get('maximum_stock') else None
            product.is_active = data.get('is_active', product.is_active)
            product.group = data.get('group') if data.get('group') else None
            
            if data.get('brand'):
                product.brand_id = data.get('brand')
            if data.get('category'):
                product.category_id = data.get('category')
            if data.get('business'):
                product.business_id = data.get('business')
...
# ViewSet simplificado para negocios/businesses
class SimpleBusinessViewSet(viewsets.ModelViewSet):
    """ViewSet simplificado para negocios"""
    permission_classes = [IsAuthenticated]
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            return Response({
                'error': f'Error al obtener negocios: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


# ViewSet simplificado para almacenes/warehouses
class SimpleWarehouseViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        try:
            warehouses = Warehouse.objects.values(
                'id',
                'name',
                'description',
                'address',
            ...
            return Response({
                'error': f'Error al obtener almacenes: {str(e)}'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)


# ViewSet simplificado para marcas/brands
class SimpleBrandViewSet(viewsets.ModelViewSet):
    """ViewSet simplificado para marcas"""
    permission_classes = [IsAuthenticated]
...
# ViewSet simplificado para categorías
class SimpleProductWarehouseStockViewSet(viewsets.ModelViewSet):
    """ViewSet simplificado para stock de productos en almacenes"""
    permission_classes = [IsAuthenticated]
    
    def get_queryset(self):
        return ProductWarehouseStock.objects.all()
    ...
    def list(self, request, *args, **kwargs):
        try:
            stocks = ProductWarehouseStock.objects.select_related(
                ...
                'warehouse__address',
                # Información de la variante de producto
                'product_variant__id',
                'product_variant__name',
                'product_variant__sku',
                'product_variant__price',
                'product_variant__min_stock'
            ).order_by('warehouse__name', 'product_variant__name')
            ...
            for stock in stocks:
                item = {
                    ...
                    # Warehouse info como objeto anidado para compatibilidad
                    'warehouse_info': {
                        'id': stock['warehouse__id'],
                        'name': stock['warehouse__name'],
                        'description': stock['warehouse__description'],
                        'address': stock['warehouse__address']
                    },
                    # Product variant info como objeto anidado
                    'product_variant_info': {
                        'id': stock['product_variant__id'],
                        'name': stock['product_variant__name'],
                        'sku': stock['product_variant__sku'],
                        'price': stock['product_variant__price'],
                        'min_stock': stock['product_variant__min_stock']
                    }
                }
                data.append(item)
            ...
            return Response(
                {'error': f'Error al obtener stock: {str(e)}'}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    
    def retrieve(self, request, pk=None):
        try:
            stock = ProductWarehouseStock.objects.select_related(
                'product_variant', 
                'warehouse'
            ).get(pk=pk)
            ...
            data = {
                'id': stock.id,
                'product_variant': stock.product_variant.id,
                ...
                'product_variant_info': {
                    ...
                    'name': stock.product_variant.name,
                    'sku': stock.product_variant.sku,
                    'price': stock.product_variant.price,
                    'min_stock': stock.product_variant.min_stock
                }
            }
    ...
    def create(self, request, *args, **kwargs):
        try:
            data = request.data
            stock = ProductWarehouseStock.objects.create(
            ...
            return Response(
                {'error': f'Error al crear stock: {str(e)}'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
    
    def update(self, request, pk=None):
        try:
            stock = ProductWarehouseStock.objects.get(pk=pk)
            data = request.data
            
            if 'quantity' in data:
                stock.quantity = data['quantity']
            if 'product_variant' in data:
                stock.product_variant_id = data['product_variant']
            if 'warehouse' in data:
                stock.warehouse_id = data['warehouse']
    ...
    def destroy(self, request, pk=None):
        try:
            ...
            return Response(
                {'error': f'Error al eliminar stock: {str(e)}'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
